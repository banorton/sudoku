...
class Sudoku_Board:
    ...
    def get_box(self, box_pos):
        box_row = box_pos[0] + 1
        box_col = box_pos[1] + 1
        box_arr = self.board_state.reshape(
            9, 9)[box_row * 3 - 3:box_row * 3, box_col * 3 - 3:box_col * 3]
        return box_arr
    ...
    def get_notes_box(self, box_pos):
        box_row = box_pos[0] + 1
        box_col = box_pos[1] + 1
        rows = [self.notes[box_row*3-3],
                self.notes[box_row*3-2], self.notes[box_row*3-1]]
        box_notes = []
        for row in rows:
        ...
        for row_num in range(9):
            for col_num in range(9):
                if self.board_state[(row_num)*9 + col_num] == 0:
                    impossible_vals = set(self.get_row(row_num)) | set(self.get_col(col_num).T) | set(
                        self.get_box(self.cell_to_box((row_num, col_num))).flatten())
                    ...
                    self.replace_note((row_num, col_num), {
                                      self.get_cell((row_num, col_num))})
            ...
            for num, positions in enumerate(num_positions):
                if len(positions) == 2:
                    ...
                    if positions[0][0] == positions[1][0]:
                        print("2 Inline Row: (" + str(num) + ") in positions " + str(
                            positions[0]) + " and " + str(positions[1]) + " in box " + str(box_num))
                        self.discard_notes_row(positions[0][0], num)
                        print("Discarding " + str(num) + " from row " +
                              str(positions[0][0]), end="\n\n")
                    ...
                    # Columns are the same.
                        ...
                        print("2 Inline Col: (" + str(num) + ") in positions " + str(
                            positions[0]) + " and " + str(positions[1]) + " in box " + str(box_num))
                        self.discard_notes_col(positions[0][1], num)
                        print("Discarding " + str(num) + " from col " +
                              str(positions[0][1]), end="\n\n")
                        self.notes[positions[0][0]][positions[0][1]].add(num)
                        self.notes[positions[1][0]][positions[1][1]].add(num)
                elif len(positions) == 3:
                    # Rows are the same.
                    if positions[0][0] == positions[1][0] == positions[2][0]:
                        print("3 Inline Row: (" + str(num) + ") in positions " + str(positions[0]) + " and " + str(
                            positions[1]) + " and " + str(positions[2]) + " in box " + str(box_num))
                        self.discard_notes_row(positions[0][0], num)
                        print("Discarding " + str(num) + " from row " +
                              str(positions[0][0]), end="\n\n")
                        self.notes[positions[0][0]][positions[0][1]].add(num)
                    ...
                    # Rows are the same.
                        ...
                        print("3 Inline Col: (" + str(num) + ") in positions " + str(positions[0]) + " and " + str(
                            positions[1]) + " and " + str(positions[2]) + " in box " + str(box_num))
                        self.discard_notes_col(positions[0][1], num)
                        print("Discarding " + str(num) + " from Col " +
                              str(positions[0][1]), end="\n\n")
            ...
            for num1, positions1 in enumerate(num_positions):
                ...
                for num2, positions2 in enumerate(num_positions):
                    ...
                    if positions1 == positions2:
                        # Double found.
                        print("Double Box: (" + str(num1) + ", " + str(num2) + ") in positions " + str(
                            positions1[0]) + " and " + str(positions1[1]) + " in box " + str(box_num))
                        box_pos = self.num_to_pos(box_num)
                        self.discard_notes_box(box_pos, num1)
                        self.discard_notes_box(box_pos, num2)
                        print("Discarding " + str(num1) + " and " +
                              str(num2) + " from box " + str(box_num))
                        # Rows are the same.
                        if positions1[0][0] == positions1[1][0]:
                            self.discard_notes_row(positions1[0][0], num1)
                            self.discard_notes_row(positions1[0][0], num2)
                            print("Discarding " + str(num1) + " and " +
                                  str(num2) + " from row " + str(positions1[0][0]))
                        # Columns are the same.
                        elif positions1[0][1] == positions1[1][1]:
                            self.discard_notes_col(positions1[0][1], num1)
                            self.discard_notes_col(positions1[0][1], num2)
                            print("Discarding " + str(num1) + " and " +
                                  str(num2) + " from col " + str(positions1[0][1]))
                        self.notes[positions1[0][0]
                                   ][positions1[0][1]] = {num1, num2}
                        self.notes[positions1[1][0]
                                   ][positions1[1][1]] = {num1, num2}
                        print()
        ...
        for row_num in range(9):
            ...
            for num1, positions1 in enumerate(num_positions):
                ...
                for num2, positions2 in enumerate(num_positions):
                    ...
                    if positions1 == positions2:
                        # Double found.
                        print("Double Row: (" + str(num1) + ", " + str(num2) +
                              ") in positions " + str(positions1[0]) + " and " + str(positions1[1]))
                        ...
                        if self.cell_to_box(positions1[0]) == self.cell_to_box(positions1[1]):
                            self.discard_notes_box(box_pos, num1)
                            self.discard_notes_box(box_pos, num2)
                            print("Discarding " + str(num1) + " and " + str(num2) +
                                  " from box " + str(self.pos_to_num(self.cell_to_box(positions1[0]))))
                        self.notes[positions1[0][0]
                                   ][positions1[0][1]] = {num1, num2}
                        self.notes[positions1[1][0]
                                   ][positions1[1][1]] = {num1, num2}
        ...
        for col_num in range(9):
            ...
            for num1, positions1 in enumerate(num_positions):
                ...
                for num2, positions2 in enumerate(num_positions):
                    ...
                    if positions1 == positions2:
                        # Double found.
                        print("Double Col: (" + str(num1) + ", " + str(num2) +
                              ") in positions " + str(positions1[0]) + " and " + str(positions1[1]))
                        box_pos = self.cell_to_box(positions1[0])
                        self.discard_notes_col(positions1[0][1], num1)
                        self.discard_notes_col(positions1[0][1], num2)
